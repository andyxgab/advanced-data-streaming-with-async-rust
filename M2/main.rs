...
#[clap(
    version = "1.0",
    author = "Andrea Gabana derived from Claus Matzinger",
...
trait AsyncStockDifference {
    ...
    /// The signal (using the provided type) or `None` on error/invalid data.
    ///
   async fn calculate(&self, series: &[f64]) -> Option<(f64, f64)>;
   
}
#[async_trait]
trait AsyncStockWindow {
...
#[async_trait]
    impl AsyncStockDifference for PriceDifference {
        type SignalType = f64;
...
#[async_trait]
    impl AsyncStockWindow for WindowedSMA {
        type SignalType = f64;
...
async fn fetch_closing_data(
    ...
    let response = provider
        ...
        .await.map_err(|_| Error::from(ErrorKind::InvalidData))?;
...
async fn main() -> std::io::Result<()> {
    ...
    for symbol in opts.symbols.split(',') {
        ...
        if !closes.is_empty() {
                ...
                let signal_max = MaxPrice {};
                let signal_min = MinPrice {};
                let prc_diff = PriceDifference {};
                let win_sma = WindowedSMA {window_size:30};
                let period_max: f64 = signal_max.calculate(&closes).await.unwrap();
                let period_min: f64 = signal_min.calculate(&closes).await.unwrap();
                let last_price = *closes.last().unwrap_or(&0.0);
                let (_, pct_change) = prc_diff.calculate(&closes).await.unwrap_or((0.0, 0.0));
                let sma = win_sma.calculate( &closes).await.unwrap_or_default();
